// 과제·사용자 데이터 조회·생성·수정·삭제 (DB 접근 계층)
import { getDb } from "@/lib/db";

// node:sqlite 가 돌려주는 행은 prototype 이 없는 객체라, 서버 컴포넌트에서
// 클라이언트 컴포넌트로 넘기려면 일반 객체로 변환해야 한다.
const toPlain = (r) => (r ? { ...r } : r);
...
export function listTasks(filters = {}) {
  ...
  `;
  return getDb().prepare(`${TASK_SELECT} ${clause} ${order}`).all(...params).map(toPlain);
}

export function getTask(id) {
  const row = getDb().prepare(`${TASK_SELECT} WHERE t.id = ?`).get(id);
  if (!row) return null;
  const task = toPlain(row);
  task.links = getDb()
