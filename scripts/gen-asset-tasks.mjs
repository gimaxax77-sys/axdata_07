// 게임(axdata_01_eldria) 로스터를 읽어, 아직 아트가 없는 캐릭터 초상만 골라 과제로 등록
//
...
//
// 사용법:
//   node scripts/gen-asset-tasks.mjs [게임경로]     (기본: ../axdata_01_eldria)
//
...
const gameRoot = path.resolve(process.argv[2] || '../axdata_01_eldria');
...
if (!fs.existsSync(conceptsEntry)) {
  ...
  console.error('사용법: node scripts/gen-asset-tasks.mjs [게임경로]  (기본: ../axdata_01_eldria)');
  process.exit(1);
}
